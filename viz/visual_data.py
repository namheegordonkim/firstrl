...
class XMLVisualDataContainer:
    ...
    def __init__(self, xml_path: str):
        ...
        for i in range(mj.ngeom):
            my_pos = mj.geom_pos[i] * 1
            # my_pos = body_global_pos[mj.geom_bodyid[i]] + mj.geom_pos[i]
            my_pos *= 0
            my_quat = body_global_quat[mj.geom_bodyid[i]]
            ...
            if mj.geom_type[i] == 2:
                ...
                # offset += direction * mj.geom_size[i][1]
            elif mj.geom_type[i] == 6:  # box
                bounds = np.stack([-mj.geom_size[i], mj.geom_size[i]], axis=-1).reshape(
                    -1
                )
                mesh = pv.Box(bounds)
            else:
...
#             local_child_offset = np.array(
#                 child.attrib.get("pos", "0 0 0").split(), dtype=float
#             )
...
#                 geom_pos = np.array(
#                     geom.attrib.get("pos", "0 0 0").split(), dtype=float
#                 )
...
#             geom_pos = np.array(
#                 geom_tag.attrib.get("pos", "0 0 0").split(), dtype=float
#             )
#
...
#                     float(k) * 2 for k in geom_tag.attrib["size"].split()
#                 ]
#                 center = np.zeros(3)
#                 mesh = pv.Cube(
#                     center=geom_pos,
...
#                 mesh = pv.Arrow(
#                     start=fro, direction=direction, scale=length, shaft_radius=size
#                 )
#
#                 self.centers.append(center)
#             else:
